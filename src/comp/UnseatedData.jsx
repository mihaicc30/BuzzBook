...
export const UnseatedData = ({ venueData }) => {
  ...
  const bookings = venueData[0]?.bookings?.filter((data, index) => data.assignedSlot == undefined).sort((a, b) => (sortByThis(a, b) ? 1 : -1));
  ...
  return (
    <Suspense>
      <div key={crypto.randomUUID()} className={`sectionContainer grid grid-cols-1 auto-rows-max overflow-auto min-w-[1450px] pb-[10vh]`}>
        <div className={`grid bg-gray-200 h-[60px] grid-cols-[80px_50px_80px_50px_50px_60px_30px_30px_200px_70px_63px_550px] items-start border-b-2 border-b-black/[2%] font-[600]`}>
          <div></div>
        ...
        {bookings &&
          ...
          bookings
            ...
            .map((booking, indx) => {
              ...
              return (
                <div key={crypto.randomUUID()} className={`grid pt-1 ${indx % 2 == 0 ? "bg-white" : "bg-gray-100"} ${booking.status.status === "Expected" ? "" : "opacity-40"} grid-cols-[80px_50px_80px_50px_50px_60px_30px_30px_200px_70px_63px_550px] items-start border-b-2 border-b-black/[2%]`}>
                  <button className="bg-orange-400 p-1 mx-1 rounded-xl text-sm font-bold" onClick={() => updateContext({ modalData: booking })}>
                    Find Seat
                  </button>
                  <p className="text-center">{booking.desiredStartTime}</p>
                  ...
                  <p className="mx-auto tracking-tighter">-</p>
